...
from functools import partial
import numpy as np
def parallel_compute_score(evaluation_func, response_str, ground_truth, data_sources, training_steps, timeout=6, max_workers=64):

    with tqdm(total=len(response_str)) as pbar:
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = {
                executor.submit(evaluation_func, response_str[index], ground_truth[index], data_sources[index], training_steps=training_steps): index
...
class YRRewardManager_hacking_penalty:
    ...
    def __call__(self, data: DataProto, global_training_steps=0):
        """We will expand this function gradually based on the available datasets"""

        # If there is rm score, we directly return rm score. Otherwise, we compute via rm_score_fn
        # print("DEBUG YR.__call__ self.compute_score:", self.compute_score, callable(self.compute_score))
