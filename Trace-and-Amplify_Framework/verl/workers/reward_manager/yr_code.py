...
def _kill_tree(pid: int, term_timeout: float = 2.0):
    """Kill entire process trees of pool workers (and their grandchildren)"""
    try:
    ...
    for q in procs:
        with suppress(Exception):
            q.terminate()
    _, alive = psutil.wait_procs(procs, timeout=term_timeout)
    for q in alive:
        with suppress(Exception):
            q.kill()
...
def parallel_compute_score(evaluation_func, response_str, ground_truth, data_sources, timeout=60, max_workers=64):
    ...
    with tqdm(total=len(response_str)) as pbar:
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = {
                executor.submit(evaluation_func, 
                                response_str[index], 
                                ground_truth[index], 
                                data_sources[index]): index
                for index in range(len(response_str))
            }
            results = {}
            metadata = {}
            ...
            procs_attr = getattr(executor, "_processes", None)
            if isinstance(procs_attr, dict):
                for pid in list(procs_attr.keys()):
                    _kill_tree(pid, term_timeout=1.5)
