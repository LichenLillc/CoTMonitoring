...
def parse_args():
    ...
    # Note: Removed the --template parameter; this script becomes a pure text extractor, decoupled from specific models
...
# Globally unique pair ID generation function
...
def process_persona_format(item):
    ...
    # [Core Modification 1]: Thoroughly clean possible end markers already attached to the end of response
    ...
    return {
        "prompt": user_content,  # Output only the clean question
        "response": ass_content, # Output only the clean code/answer
...
def process_instruct_format(item):
    ...
    return {
        "prompt": user_content,  # Output only the clean question
        "response": response,    # Output only the clean code/answer
        "hacking_label": item.get("hacking_label", 0),
        "hacking_type": item.get("hacking_type", "unknown"),
        "traj_source": item.get("traj_source", "unknown"),
        "pair_id": pair_id
