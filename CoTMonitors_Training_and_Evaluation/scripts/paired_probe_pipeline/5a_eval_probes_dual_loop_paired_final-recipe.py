...
import os
# Prevent CPU overload in multiprocessing
...
# ==========================================
# 1. MLP Definitions (keep all class definitions to prevent errors)
# ==========================================
...
def evaluate_model_seed_all_datasets(m_path, row_key, dataset_info_list, seed):
    ...
    for d_path_str, col_key in dataset_info_list:
        ...
        except ValueError: continue # Soft shield: skip on dimension mismatch
...
def process_modality(mgp_path, dgp_path, modality_name, args):
    ...
    # Isolate the reports directory and the cache directory
        ...
        for dp in dps:
            ...
            if cache_file.exists():
                ...
                for ml1, ml2, m_path, id_vals in model_rows:
                    row_key = (ml1, ml2)
                    for mode in ["raw", "pca"]:
                        # [Minimal Modification]: Dictionary preallocation only handles these two models
                        for m_type in ["logreg", "mlp_v1"]:
...
def main():
    ...
    parser.add_argument("--mgp_text", type=str, default="./main_table3_paired/exp_0328/probe_outputs_text")
    parser.add_argument("--dgp_text", type=str, default="./main_table3_paired/exp_0328/processed_text")
    parser.add_argument("--mgp_eos", type=str, default="./main_table3_paired/exp_0328/probe_outputs_eos")
    parser.add_argument("--dgp_eos", type=str, default="./main_table3_paired/exp_0328/processed_eos")
    parser.add_argument("--seed", type=int, default=None)
    parser.add_argument("--results_root", type=str, default="./main_table3_paired/exp_0328/5a_results/")
